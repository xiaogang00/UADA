...
from model.resnet import ResNet18_class100
from model.wideresnet2 import WideResNet

from model.smooth_cross_entropy import smooth_crossentropy
from data.cifar100_noaug import Cifar
...
import os
from utility.cutout import Cutout2
...
from utility.autoaug3 import CIFARPolicy

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--batch_size", default=128, type=int, help="Batch size used in the training and validation loop.")
    parser.add_argument("--num_choose", default=2, type=int)
    ...
    log = Log(log_each=10)
    # model = ResNet18_class100().to(device) ### ResNet18
    model = WideResNet(args.depth, 100, widen_factor=args.width_factor, dropRate=0.3).to(device) ### WideResNet
    ...
    mean, std = dataset._get_statistics()
    cut_operation = Cutout2()
    crop_operation = Random_Crop(32, 32, padding=(4, 4, 4, 4))
    auto_operation = CIFARPolicy()
    ...
    for epoch in range(start_epoch, args.epochs, 1):
        ...
        for batch in dataset.train:
            ...
            for mm in range(inputs_origin.shape[0]):
                ...
                inputs_this, left, top, right, bottom = cut_operation.forward(inputs_this)
                ...
                cutout_parameter.append([left, top, right, bottom])
                ...
                inputs_this2 = cut_operation.forward2(inputs_this2, left, top, right, bottom)
                ...
                inputs_this3 = cut_operation.forward2(inputs_this3, left, top, right, bottom)
                ...
                inputs_this4 = cut_operation.forward2(inputs_this4, left, top, right, bottom)
                ...
                #############################################################################
                ### adversarial perturbation for the data augmentation of cutout
                i_new = i
                j_new = j
                left_new = left + random.choice([-1, 1])
                top_new = top + random.choice([-1, 1])
                right_new = right + random.choice([-1, 1])
                bottom_new = bottom + random.choice([-1, 1])
                inputs_this5 = crop_operation.forward2(inputs_this_origin.clone(), i_new, j_new)
                ...
                inputs_this5 = auto_operation.forward2(inputs_this5, magnitude1_list, random_value_list, para_list)
                ...
                inputs_this5 = cut_operation.forward2(inputs_this5, left_new, top_new, right_new, bottom_new)
                inputs_this5 = (inputs_this5 - mean_this) * 1.0 / std_this
                inputs5.append(inputs_this5.unsqueeze(dim=0))
                cutout_parameter_new.append([left_new, top_new, right_new, bottom_new])
            ...
            inputs5 = torch.cat(inputs5, dim=0).cuda()
            ...
            with torch.no_grad():
                ...
                predictions5 = model(inputs5)
                loss5 = F.cross_entropy(predictions5, targets, reduce=False)
            batch_size = inputs2.shape[0]
            distance1 = loss2 - loss1
            distance2 = loss3 - loss1
            distance3 = loss4 - loss1
            distance4 = loss5 - loss1
            ...
            for mm in range(inputs_origin.shape[0]):
                ...
                left, top, right, bottom = cutout_parameter[mm]
                left_new, top_new, right_new, bottom_new = cutout_parameter_new[mm]
                ...
                inputs_this5 = inputs_this5.permute(2, 0, 1)
                inputs_this5 = inputs_this5 * 1.0 / 255.0
                inputs_this5 = cut_operation.forward2(inputs_this5, left, top, right, bottom)
                inputs_this5 = (inputs_this5 - mean_this) * 1.0 / std_this
                inputsss.append(inputs_this5.unsqueeze(dim=0))

                ########################################################################################
                ### according to the gradient, determine the value of data augmentation of cutout
                left_this = left + alpha * np.sign(distance4[mm].item() * 1.0 / (left_new - left))
                top_this = top + alpha * np.sign(distance4[mm].item() * 1.0 / (top_new - top))
                right_this = right + alpha * np.sign(distance4[mm].item() * 1.0 / (right_new - right))
                bottom_this = bottom + alpha * np.sign(distance4[mm].item() * 1.0 / (bottom_new - bottom))
                left_this = int(left_this)
                top_this = int(top_this)
                right_this = int(right_this)
                bottom_this = int(bottom_this)
                i_this = i
                j_this = j
                i_this = int(i_this)
                j_this = int(j_this)
                inputs_this6 = crop_operation.forward2(inputs_this_origin.clone(), i_this, j_this)
                inputs_this6 = inputs_this6[0]
                inputs_this6 = inputs_this6.cpu().permute(1, 2, 0).numpy() * 255.0
                inputs_this6 = inputs_this6.astype(np.uint8)
                if flip_random_origin:
                    inputs_this6 = cv2.flip(inputs_this6, 1)
                inputs_this6 = PIL.Image.fromarray(inputs_this6)
                inputs_this6 = auto_operation.forward2(inputs_this6, magnitude1_list, random_value_list, para_list)
                inputs_this6 = np.array(inputs_this6)
                inputs_this6 = torch.Tensor(inputs_this6).float()
                inputs_this6 = inputs_this6.permute(2, 0, 1)
                inputs_this6 = inputs_this6 * 1.0 / 255.0
                inputs_this6 = cut_operation.forward2(inputs_this6, left_this, top_this, right_this, bottom_this)
                inputs_this6 = (inputs_this6 - mean_this) * 1.0 / std_this
                inputssss.append(inputs_this6.unsqueeze(dim=0))


            inputs = torch.cat(inputs, dim=0).cuda()
            inputss = torch.cat(inputss, dim=0).cuda()
            inputsss = torch.cat(inputsss, dim=0).cuda()
            inputssss = torch.cat(inputssss, dim=0).cuda()
            with torch.no_grad():
                ...
                predictions_adv3 = model(inputssss)
                loss_adv3 = F.cross_entropy(predictions_adv3, targets, reduce=False)

            ### determine the data operations with the maximal loss value
            input_final = []
            for mm in range(inputs_origin.shape[0]):
                list_this = [loss_adv[mm].item(), loss_adv1[mm].item(), loss_adv2[mm].item(), loss_adv3[mm].item(), loss1[mm].item()]
                ...
                if max == 0:
                    ...
                    input_final.append(inputssss[mm:mm + 1])
                else:
